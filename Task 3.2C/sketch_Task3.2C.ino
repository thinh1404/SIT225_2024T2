...
void loop() {
  ...
  if (distance < min_threshold || distance > max_threshold) // If the object is too near the sensor  or exceeds the maximum threshold, it will trigger the alarm.
  {
    triggered = true;
  }
  else {
...
/*
  Since RandomTemperature is READ_WRITE variable, onRandomTemperatureChange() is
  executed every time a new value is received from IoT Cloud.
*/
